...
def processingIncidenceMatrix(geneList, dataPath):
    ...
    for id in ids:
        ...
        incidenceMatrixTemp = pd.DataFrame(data=genesetData.toarray(), index=geneList)
...
def create_dynamic_prototypes(train_idx, label_tensor, incidence_matrix, model, device):
    ...
    positive_genes = train_genes[(train_labels == 1).cpu().numpy()]
    negative_genes = train_genes[(train_labels == 0).cpu().numpy()]
